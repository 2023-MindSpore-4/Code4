...
class ResNeXtBottleneck(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, stride,
        ...
        self.conv_reduce = nn.Conv2d(
            in_channels, D, kernel_size=1, stride=1, padding=0, has_bias=False)
        self.bn_reduce = nn.BatchNorm2d(D, momentum=0.001)
        self.conv_conv = nn.Conv2d(D, D,
                                   kernel_size=3, stride=stride, padding=1,
                                   groups=cardinality, has_bias=False)
        self.bn = nn.BatchNorm2d(D, momentum=0.001)
        self.act = mish
        self.conv_expand = nn.Conv2d(
            D, out_channels, kernel_size=1, stride=1, padding=0, has_bias=False)
        self.bn_expand = nn.BatchNorm2d(out_channels, momentum=0.001)

        self.shortcut = nn.SequentialCell()
        if in_channels != out_channels:
            self.shortcut.insert_child_to_cell('shortcut_conv',
                                     nn.Conv2d(in_channels, out_channels,
                                               kernel_size=1,
                                               stride=stride,
                                               padding=0,
                                               has_bias=False))
            self.shortcut.insert_child_to_cell(
...
class CifarResNeXt(nn.Cell):
    ...
    def __init__(self, cardinality, depth, num_classes,
        ...
        self.classifier = nn.Dense(self.stages[3], num_classes)
    ...
    def block(self, name, in_channels, out_channels, pool_stride=2):
        ...
        for bottleneck in range(self.block_depth):
            ...
            if bottleneck == 0:
                block.insert_child_to_cell(name_, ResNeXtBottleneck(in_channels,
                                                          out_channels,
                                                          pool_stride,
                                                          self.cardinality,
                                                          self.base_width,
                ...
                block.insert_child_to_cell(name_,
