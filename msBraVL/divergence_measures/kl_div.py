...
def calc_kl_divergence(mu0, logvar0, mu1=None, logvar1=None, norm_value=None):
    logvar0 = ms.Tensor(input_data=logvar0, dtype=ms.float32)
    # logvar1 = ms.Tensor(input_data=logvar1, dtype=ms.float32)
    if mu1 is None or logvar1 is None:
        KLD = -0.5 * ms.ops.sum(1 - logvar0.exp() - mu0.pow(2) + logvar0)
    else:
        KLD = -0.5 * ms.ops.sum(1 - logvar0.exp() / logvar1.exp() - (mu0-mu1).pow(2) / logvar1.exp() + logvar0 - logvar1)
