...
def poe(mu, logvar, eps=1e-8):
    var = ms.ops.exp(logvar) + eps
    # precision of i-th Gaussian expert at point x
    T = 1. / var
    pd_mu = ms.ops.sum(mu * T, dim=0) / ms.ops.sum(T, dim=0)
    pd_var = 1. / ms.ops.sum(T, dim=0)
    pd_logvar = ms.ops.log(pd_var)
...
def alpha_poe(alpha, mu, logvar, eps=1e-8):
    ...
    T = 1 / var
    pd_var = 1. / ms.ops.sum(alpha_expanded * T, dim=0)
    pd_mu = pd_var * ms.ops.sum(alpha_expanded * mu * T, dim=0)
    pd_logvar = ms.ops.log(pd_var)
    return pd_mu, pd_logvar
...
def calc_alphaJSD_modalities_mixture(m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    klds = ops.zeros(2)
    entropies_mixture = ops.zeros(2)
    w_modalities = ms.ops.Tensor(flags.alpha_modalities[1:])
...
def calc_alphaJSD_modalities(flags, mus, logvars, weights, normalization=None):
    ...
    if normalization is not None:
        klds = ops.zeros(num_mods)
    else:
        klds = ops.zeros(num_mods, num_samples)
    klds = klds
...
def calc_group_divergence_moe(flags, mus, logvars, weights, normalization=None):
    ...
    weights = weights
    for k in range(0, num_mods):
        kld_ind = calc_kl_divergence(mus[k, :, :], logvars[k, :, :], norm_value=normalization)
        if normalization is not None:
            klds[k] = kld_ind
        else:
            klds[k, :] = kld_ind
...
def calc_modality_divergence(m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    if flags.modality_poe:
        ...
        uniform_mu = ops.zeros(m1_mu.shape)
        uniform_logvar = ops.zeros(m1_logvar.shape)
        klds = ops.zeros(3, 3)
        klds_modonly = ops.zeros(2, 2)
