...
def log_mean_exp(value, dim=0, keepdim=False):
    return ms.ops.logsumexp(value, axis=dim, keep_dims=keepdim) - math.log(value.shape[dim])


def m_cubo(x, qz_x, px_zs, zss):
    lpz = dist.Normal(ms.ops.zeros((1, zss.shape[2]), dtype=ms.float32),
                      ms.ops.ones((1, zss.shape[2]), dtype=ms.float32)).log_prob(zss).sum(-1)
    ...
    if 'brain' in px_zs.keys() and 'image' in px_zs.keys() and 'text' in px_zs.keys():
        ...
        lpx_z1 = px_zs['image'].log_prob(x['image']).sum(-1)
        lpx_z2 = px_zs['text'].log_prob(x['text']).sum(-1)
        temp = 2 * (lpz + lpx_z1 + lpx_z2 - lqz_x)
        cubo = 0.5 * log_mean_exp(ms.Tensor(temp))
    return cubo.mean()
...
def log_li(x_var, dist_info):
    ...
    pi = ms.Tensor(torch.ones(1) * np.pi)
...
def mutual_info(exp, px_zs, z):
    if 'brain' in px_zs.keys() and 'image' in px_zs.keys() and 'text' in px_zs.keys():
        q1 = exp.Q1(px_zs['brain'].mean())
        q2 = exp.Q2(px_zs['image'].mean())
        q3 = exp.Q3(px_zs['text'].mean())
...
def negative_sample_generator(batch_new, batch, case):
    ...
    if 'brain' in batch_d.keys() and 'image' in batch_d.keys() and 'text' in batch_d.keys():
        if case == 1:
            ...
            idx = ms.Tensor(np.unique(np.concatenate((idx1.numpy(), idx2.numpy()), axis=0)))
            # print('idx:{},idx的形状{}:'.format(idx, idx.shape))
...
def basic_routine_epoch(exp, batch, epoch):
    ...
    for k, m_key in enumerate(batch_d.keys()):
        batch_d[m_key] = ms.Tensor(batch_d[m_key])
...
def elbo_contrast(exp, batch, epoch):
    ...
    out_basic_routine = dict()
    out_basic_routine['elbo_nega_sample_loss'] = torch.log(
        elbo_scale.exp().sum() * exp.flags.batch_size / neg_batch_size + TINY) * CONSTANT
    return out_basic_routine
...
def update_Qnet(exp, batch):
    ...
    batch_d = batch[0]
    for k, m_key in enumerate(batch_d.keys()):
        batch_d[m_key] = torch.Tensor(batch_d[m_key])
    # print(batch_d)
    results = mm_vae(batch_d)
    z = results['class_embeddings']
    px_zs = results['rec']
...
def train_aug(epoch, exp):
    ...
    if exp.flags.aug_type == 'image_text':
        ...
        aug_loader = DataLoader(exp.dataset_aug, batch_size=exp.flags.batch_size, shuffle=True, num_workers=8,
                                drop_last=True)
        # grad_fn = ms.ops.value_and_grad(mm_vae.forward(), op)
        for iteration, batch in enumerate(aug_loader):
            ...
            # print('new:', batch_new, 'raw:', batch)
            # Stage 1
            intra_mi = update_Qnet(exp, batch_new)
            # Stage 2
            basic_routine = basic_routine_epoch(exp, batch_new, epoch)
            total_loss = (basic_routine['total_loss']).asnumpy()
            elbo_loss = (basic_routine['elbo_loss']).asnumpy()
            ...
            elbo_nega_sample_loss_case_1 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()
            neg_batch_new = negative_sample_generator(batch_new, batch, case=2)
            basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
            elbo_nega_sample_loss_case_2 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()
...
def train(epoch, exp):
    ...
    test_loader = DataLoader(exp.dataset_test, batch_size=exp.flags.batch_size, shuffle=True, num_workers=8,
                             drop_last=True)
    ...
    for iteration, batch in enumerate(d_loader):
        ...
        intra_mi = (basic_routine['intra_mi']).asnumpy()
        elbo_loss = (basic_routine['elbo_loss']).asnumpy()
        neg_batch_new = negative_sample_generator(batch_new, batch, case=1)
        basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
        elbo_nega_sample_loss_case_1 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()
        ...
        basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
        elbo_nega_sample_loss_case_3 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()

        neg_batch_new = negative_sample_generator(batch_new, batch, case=4)
        basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
        elbo_nega_sample_loss_case_4 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()

        neg_batch_new = negative_sample_generator(batch_new, batch, case=5)
        basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
        elbo_nega_sample_loss_case_5 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()

        neg_batch_new = negative_sample_generator(batch_new, batch, case=6)
        basic_routine_contrast = elbo_contrast(exp, neg_batch_new, epoch)
        elbo_nega_sample_loss_case_6 = basic_routine_contrast['elbo_nega_sample_loss'].asnumpy()
        ...
        print('====> Epoch: {:03d} Train loss: {:.4f} ELBO: {:.4f} IntraMI: {:.4f} InterMI: {:.4f}'
              .format(epoch, total_loss, -elbo_loss, -intra_mi, -inter_mi_loss))


def test(epoch, exp):
    mm_vae = exp.mm_vae
    exp.mm_vae = mm_vae
    mm_vae.set_train(False)
    lambda2 = exp.flags.lambda2
    ...
    for iteration, batch in enumerate(d_loader):
        ...
        inter_mi_loss = elbo_loss + elbo_nega_sample_loss / 6.0
        total_loss = total_loss + lambda2 * inter_mi_loss
        print('====> Epoch: {:03d} Test loss: {:.4f} ELBO: {:.4f} IntraMI: {:.4f} InterMI: {:.4f}'
              .format(epoch, total_loss, -elbo_loss, -intra_mi, -inter_mi_loss))


def image_text_inference(exp, type_):
    mm_vae = exp.mm_vae
    mm_vae.set_train(False)
    exp.mm_vae = mm_vae
    if type_ == 'zsl':
        image_text_data = {'image': exp.dataset_test.tensors[1], 'text': exp.dataset_test.tensors[2]}
        label = exp.dataset_test.tensors[3]
        brain = exp.dataset_test.tensors[0]
    elif type_ == 'normal':
        image_text_data = {'image': exp.dataset_val.tensors[1], 'text': exp.dataset_val.tensors[2]}
        label = exp.dataset_val.tensors[3]
        brain = exp.dataset_test.tensors[0]
    results = mm_vae(image_text_data)
    z = results['class_embeddings']
    brain_rec = mm_vae.lhoods['brain'](*mm_vae.decoders['brain'](None, z))
    return z.cpu().numpy(), label.cpu().numpy(), brain_rec.mean().numpy(), brain.numpy()


def brain_inference(exp, type_):
    mm_vae = exp.mm_vae
    mm_vae.set_train(False)
    exp.mm_vae = mm_vae
    if type_ == 'zsl':
        data = {'brain': exp.dataset_test.tensors[0]}
        label = exp.dataset_test.tensors[3]
        image = exp.dataset_test.tensors[1]
        text = exp.dataset_test.tensors[2]
    elif type_ == 'normal':
        data = {'brain': exp.dataset_val.tensors[0]}
        label = exp.dataset_val.tensors[3]
        image = exp.dataset_test.tensors[1]
        text = exp.dataset_test.tensors[2]
    results = mm_vae(data)
    ...
    return z.cpu().numpy(), label.cpu().numpy(), image_rec.mean().numpy(), text_rec.mean().numpy(), image.cpu().numpy(), text.cpu().numpy()
...
def image_inference(exp, type):
    ...
    exp.mm_vae = mm_vae
    if type == 'zsl':
        data = {'image': exp.dataset_test.tensors[1]}
        label = exp.dataset_test.tensors[3]
    elif type == 'normal':
        data = {'image': exp.dataset_val.tensors[1]}
        label = exp.dataset_val.tensors[3]
    results = mm_vae(data)
    z = results['class_embeddings']
...
def text_inference(exp, type):
    ...
    exp.mm_vae = mm_vae
    if type == 'zsl':
        data = {'text': exp.dataset_test.tensors[2]}
        label = exp.dataset_test.tensors[3]
    elif type == 'normal':
        data = {'text': exp.dataset_val.tensors[2]}
        label = exp.dataset_val.tensors[3]
    results = mm_vae(data)
    z = results['class_embeddings']
    return z.cpu().numpy(), label.cpu().numpy()
...
def run_epochs_trimodal(exp):
    ...
    for epoch in range(exp.flags.start_epoch, exp.flags.end_epoch):
        ...
        train(epoch, exp)
        # save checkpoints after every 1 epoch
    ...
    if exp.flags.test_type == 'normal':
        ...
        path = './results/' + exp.flags.dataname + '_' + exp.flags.sbj + '_' + exp.flags.roi + '_' + exp.flags.aug_type + '_' + exp.flags.text_model + '_' + \
               exp.flags.image_model.split('/')[-1] + '_' + str(exp.flags.lambda1) + '_' + str(
            exp.flags.lambda2) + '_' + '_' + str(exp.flags.class_dim) + '_' + exp.flags.method + '_image_text.csv'
